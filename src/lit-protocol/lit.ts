import { DiscordProvider, GoogleProvider, EthWalletProvider, WebAuthnProvider, LitAuthClient } from "@lit-protocol/lit-auth-client";
import { AuthMethodType, ProviderType, AuthMethodScope } from "@lit-protocol/constants";
import { AuthMethod, GetSessionSigsProps, IRelayPKP, SessionSigs } from "@lit-protocol/types";

export const DOMAIN = process.env.NEXT_PUBLIC_DOMAIN || "localhost";
export const ORIGIN = process.env.NEXT_PUBLIC_VERCEL_ENV === "production" ? `https://${DOMAIN}` : `http://${DOMAIN}:3000`;

export const litAuthClient: LitAuthClient = new LitAuthClient({
  litRelayConfig: {
    relayUrl: "https://relayer-server-staging-cayenne.getlit.dev",
    relayApiKey: "LL1WJSDZ-PPL1-0YN5-B221-HJ65BNIIO971_Thunder",
  },
  litOtpConfig: {
 ...
 */
export async function authenticateWithEthWallet(address?: string, signMessage?: (message: string) => Promise<string>): Promise<AuthMethod | undefined> {
 ...
 */
export async function getSessionSigs({ pkpPublicKey, authMethod, sessionSigsParams }: { pkpPublicKey: string; authMethod: AuthMethod; sessionSigsParams: GetSessionSigsProps }): Promise<SessionSigs> {
...
export async function mintPKP(authMethod: AuthMethod): Promise<IRelayPKP> {
  const provider = getProviderByAuthMethod(authMethod);
  if (!provider) return { tokenId: "", publicKey: "", ethAddress: "" };
