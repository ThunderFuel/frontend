...
export default {
  ...
  async setTokensOwner({ tokenIds, walletAddress }: { tokenIds: any[]; walletAddress: string }): Promise<ApiResponse<any>> {
    return ThunderURL.post("v1/token/settokensowner", tokenIds, { params: { walletAddress } });
  },
};
