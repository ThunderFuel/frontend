...
async function setup(contractId: string, provider: string, wallet?: string | WalletLocked): Promise<StrategyFixedPriceSaleAbi> {
...
export async function setProtocolFee(contractId: string, provider: string, wallet: string | WalletLocked, fee: BigNumberish) {
  try {
    const contract = await setup(contractId, provider, wallet);
    const { transactionResult, transactionResponse } = await contract.functions.set_protocol_fee(fee).txParams({ gasPrice: 1 }).call();
    return { transactionResponse, transactionResult };
  } catch (err: any) {
    console.error("Strategy: " + err);
    return { err };
...
export async function getProtocolFee(contractId: string, provider: string) {
  try {
    const contract = await setup(contractId, provider);
    const { value } = await contract.functions.get_protocol_fee().get();
    return { value };
  } catch (err: any) {
    console.error("Strategy: " + err);
    return { err };
...
export async function getMakerOrderOfUser(contractId: string, provider: string, user: string, nonce: BigNumberish, isBuyOrder: boolean) {
  try {
    let side: SideInput;
    isBuyOrder ? (side = { Buy: [] }) : (side = { Sell: [] });
    const _user: AddressInput = { value: user };
    const contract = await setup(contractId, provider);
    const { value } = await contract.functions.get_maker_order_of_user(_user, nonce, side).get();
    return { value };
  } catch (err: any) {
    console.error("Strategy: " + err);
    return { err };
...
export async function isValidOrder(contractId: string, provider: string, user: string, nonce: BigNumberish, isBuyOrder: boolean) {
  try {
    let side: SideInput;
    isBuyOrder ? (side = { Buy: [] }) : (side = { Sell: [] });
    const _user: AddressInput = { value: user };
    const contract = await setup(contractId, provider);
    const { value } = await contract.functions.is_valid_order(_user, nonce, side).get();
    return { value };
  } catch (err: any) {
    console.error("Strategy: " + err);
    return { err };
...
export async function getOrderNonceOfUser(contractId: string, provider: string, user: string, isBuyOrder: boolean) {
  try {
    let side: SideInput;
    isBuyOrder ? (side = { Buy: [] }) : (side = { Sell: [] });
    const _user: AddressInput = { value: user };
    const contract = await setup(contractId, provider);
    const { value } = await contract.functions.get_order_nonce_of_user(_user, side).get();
    return { value };
  } catch (err: any) {
    console.error("Strategy: " + err);
    return { err };
...
export async function getMinOrderNonceOfUser(contractId: string, provider: string, user: string, isBuyOrder: boolean) {
  try {
    let side: SideInput;
    isBuyOrder ? (side = { Buy: [] }) : (side = { Sell: [] });
    const _user: AddressInput = { value: user };
    const contract = await setup(contractId, provider);
    const { value } = await contract.functions.get_min_order_nonce_of_user(_user, side).get();
    return { value };
  } catch (err: any) {
    console.error("Strategy: " + err);
    return { err };
