...
const main = async (provider: Provider) => {
    ...
    const deployAll = async () => {
        ...
        const { transactionResult: tmInit } = await TransferManager.initialize(
        ...
        const { transactionResult: tsInit } = await TransferSelector.initialize(
        ...
        // Initialize NFT
        const { transactionResult: erc721Init } = await ERC721.initialize(
            erc721.id.toString(),
            provider.url,
            OWNER.privateKey,
            10000,
        ...
        const { transactionResult: mintTx } = await nftContract.functions.mint(20, { Address: { value: OWNER.address.toB256() } })
            .txParams({gasPrice: 1})
            .call()
