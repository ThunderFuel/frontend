...
export async function totalSupply(contractId: string, provider: string, asset: string) {
  try {
    ...
    const { value } = await contract.functions.total_supply(_asset).simulate();
    return { value };
  } catch (err: any) {
...
export async function balanceOf(contractId: string, provider: string, wallet: string | WalletLocked, account: string, asset: string) {
  try {
    const _account: IdentityInput = { Address: { value: account } };
    const _asset: ContractIdInput = { value: asset };
    const contract = await setup(contractId, provider, wallet);
    const { value } = await contract.functions.balance_of(_account, _asset).simulate();
    return { value };
  } catch (err: any) {
    console.error("Pool: " + err);
    return { err };
...
export async function getTransferManager(contractId: string, provider: string) {
  try {
    const contract = await setup(contractId, provider);
    const { value } = await contract.functions.get_asset_manager().simulate();
    return { value };
  } catch (err: any) {
...
export async function getExchange(contractId: string, provider: string) {
  try {
    const contract = await setup(contractId, provider);
    const { value } = await contract.functions.get_exchange().simulate();
    return { value };
  } catch (err: any) {
