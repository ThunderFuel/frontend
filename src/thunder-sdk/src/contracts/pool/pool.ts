...
async function setup(contractId: string, provider: string, wallet?: string | WalletLocked): Promise<PoolAbi> {
...
export async function initialize(contractId: string, provider: string, wallet: string | WalletLocked, exchange: string, assetManager: string) {
  try {
    const contract = await setup(contractId, provider, wallet);
    const _exchange: ContractIdInput = { value: exchange };
    const _Pool: ContractIdInput = { value: assetManager };
    const { transactionResult, transactionResponse } = await contract.functions.initialize(_exchange, _Pool).txParams({ gasPrice: 1 }).call();
...
export async function name(contractId: string, provider: string) {
  try {
    const contract = await setup(contractId, provider);
    const { value } = await contract.functions.name().get();
    return { value };
  } catch (err: any) {
...
export async function symbol(contractId: string, provider: string) {
  try {
    const contract = await setup(contractId, provider);
    const { value } = await contract.functions.symbol().get();
    return { value };
  } catch (err: any) {
    console.error("Pool: " + err);
    return { err };
...
export async function totalSupply(contractId: string, provider: string, asset: string) {
  try {
    ...
    const contract = await setup(contractId, provider);
    const { value } = await contract.functions.total_supply(_asset).get();
    return { value };
  } catch (err: any) {
    console.error("Pool: " + err);
    return { err };
...
export async function balanceOf(contractId: string, provider: string, account: string, asset: string) {
  try {
    const _account: IdentityInput = { Address: { value: account } };
    const _asset: ContractIdInput = { value: asset };
    const contract = await setup(contractId, provider);
    const { value } = await contract.functions.balance_of(_account, _asset).get();
    return { value };
  } catch (err: any) {
    console.error("Pool: " + err);
    return { err };
...
export async function deposit(contractId: string, provider: string, wallet: string | WalletLocked, amount: BigNumberish, assetId: BytesLike, assetManagerAddr: string) {
  try {
    const contract = await setup(contractId, provider, wallet);
    ...
    const { transactionResponse, transactionResult } = await contract.functions.deposit().txParams({ gasPrice: 1 }).addContracts([assetManager]).callParams({ forward: coin }).call();
    return { transactionResponse, transactionResult };
  } catch (err: any) {
...
export async function withdraw(contractId: string, provider: string, wallet: string | WalletLocked, amount: BigNumberish, assetId: string, assetManagerAddr: string) {
  try {
    const contract = await setup(contractId, provider, wallet);
    const _asset: ContractIdInput = { value: assetId };
    const _provider = new Provider(provider);
    const assetManager = new Contract(assetManagerAddr, AssetManagerAbi__factory.abi, _provider);
    const { transactionResponse, transactionResult } = await contract.functions.withdraw(_asset, amount).txParams({ gasPrice: 1, variableOutputs: 1 }).addContracts([assetManager]).call();
    return { transactionResponse, transactionResult };
  } catch (err: any) {
...
export async function withdrawAll(contractId: string, provider: string, wallet: string | WalletLocked, assetManagerAddr: string) {
  try {
    const contract = await setup(contractId, provider, wallet);
    const _provider = new Provider(provider);
    const assetManager = new Contract(assetManagerAddr, AssetManagerAbi__factory.abi, _provider);
    const { transactionResponse, transactionResult } = await contract.functions.withdraw_all().txParams({ gasPrice: 1, variableOutputs: 1 }).addContracts([assetManager]).call();
    return { transactionResponse, transactionResult };
  } catch (err: any) {
...
export async function transferFrom(contractId: string, provider: string, wallet: string | WalletLocked, from: string, to: string, asset: string, amount: BigNumberish) {
  try {
    const contract = await setup(contractId, provider, wallet);
    const _from: IdentityInput = { Address: { value: from } };
    const _to: IdentityInput = { Address: { value: to } };
    const _asset: ContractIdInput = { value: asset };
    const { transactionResponse, transactionResult } = await contract.functions.transfer_from(_from, _to, _asset, amount).txParams({ gasPrice: 1 }).call();
    return { transactionResponse, transactionResult };
  } catch (err: any) {
...
export async function setAssetManager(contractId: string, provider: string, wallet: string | WalletLocked, assetManager: string) {
  try {
    const contract = await setup(contractId, provider, wallet);
    const _Pool: ContractIdInput = { value: assetManager };
    const { transactionResponse, transactionResult } = await contract.functions.set_asset_manager(_Pool).txParams({ gasPrice: 1, variableOutputs: 1 }).call();
    return { transactionResponse, transactionResult };
  } catch (err: any) {
...
export async function getTransferManager(contractId: string, provider: string) {
  try {
    const contract = await setup(contractId, provider);
