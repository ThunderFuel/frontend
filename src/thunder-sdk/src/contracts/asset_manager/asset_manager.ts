...
async function setup(contractId: string, provider: string, wallet?: string | WalletLocked): Promise<AssetManagerAbi> {
...
export async function addAsset(contractId: string, provider: string, wallet: string | WalletLocked, asset: string) {
  try {
    const contract = await setup(contractId, provider, wallet);
    const _asset: ContractIdInput = { value: asset };
    const { transactionResult, transactionResponse } = await contract.functions.add_asset(_asset).txParams({ gasPrice: 1 }).call();
    return { transactionResponse, transactionResult };
  } catch (err: any) {
...
export async function removeAsset(contractId: string, provider: string, wallet: string | WalletLocked, asset: string) {
  try {
    const contract = await setup(contractId, provider, wallet);
    const _asset: ContractIdInput = { value: asset };
    const { transactionResult, transactionResponse } = await contract.functions.remove_asset(_asset).txParams({ gasPrice: 1 }).call();
    return { transactionResponse, transactionResult };
  } catch (err: any) {
...
export async function isAssetSupported(contractId: string, provider: string, asset: string) {
  try {
    const contract = await setup(contractId, provider);
    const _asset: ContractIdInput = { value: asset };
    const { value } = await contract.functions.is_asset_supported(_asset).get();
    return { value };
  } catch (err: any) {
...
export async function getSupportedAsset(contractId: string, provider: string, index: BigNumberish) {
  try {
    const contract = await setup(contractId, provider);
    const { value } = await contract.functions.get_supported_asset(index).get();
    return { value };
  } catch (err: any) {
    console.error("AssetManager: " + err);
    return { err };
...
export async function getCountSupportedAssets(contractId: string, provider: string) {
  try {
    const contract = await setup(contractId, provider);
    const { value } = await contract.functions.get_count_supported_assets().get();
    return { value };
  } catch (err: any) {
    console.error("AssetManager: " + err);
    return { err };
